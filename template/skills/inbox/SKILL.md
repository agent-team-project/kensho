...
- `send` first uses `AGENT_TEAM_DAEMON_URL` with `Authorization: Bearer $(<"$AGENT_TEAM_DAEMON_TOKEN_FILE")` when set. If that endpoint cannot be resolved or connected, it falls back to `AGENT_TEAM_DAEMON_SOCKET` (or `$AGENT_TEAM_ROOT/daemon.sock`). HTTP responses, token errors, timeouts, and ambiguous post-connect failures never fall back. The host portion of Unix-socket URLs doesn't matter — the unix dial overrides it.
