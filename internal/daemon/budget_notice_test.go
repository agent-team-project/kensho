...
func TestBudgetNoticeWritesEventsAndMailboxForCodexTokenCrossing(t *testing.T) {
	...
	if got, want := updated.Steps[0].TokenBudgetNotices, []int{50, 80}; !reflect.DeepEqual(got, want) {
	...
	if len(events) != 2 || events[0].Type != "budget_notice" || events[1].Type != "budget_notice" {
		t.Fatalf("events = %+v, want two budget_notice events", events)
	}
	if events[0].Data["dimension"] != "tokens" || events[0].Data["level"] != "50" || events[1].Data["level"] != "80" || events[0].Data["step"] != "implement" {
		t.Fatalf("event data = %+v / %+v", events[0].Data, events[1].Data)
	}
	messages, err := ReadMessages(DaemonRoot(teamDir), "worker-squ-104")
	if err != nil {
		t.Fatalf("read messages: %v", err)
	}
	if len(messages) != 2 || !strings.Contains(messages[0].Body, "budget_notice") || !strings.Contains(messages[0].Body, "agent-team budget status --self") {
