...
Add `--timeout-pipeline` with either timeout mode to keep stale-work repair scoped to one workflow; when `--timeout-jobs` is active, add `--timeout-target-agent` to scope by agent role. Team repair accepts the same timeout filters inside the team's ownership boundary.
