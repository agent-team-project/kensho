...
When `--timeout-jobs` is active, add `--timeout-pipeline` or `--timeout-target-agent` to keep stale-work repair scoped to one workflow or agent role; team repair accepts the same timeout filters inside the team's ownership boundary.
