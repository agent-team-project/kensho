...
Use `team timeout <team> --dry-run` for the same stale running-step expiration flow as `pipeline timeout`, scoped to the pipelines declared on that team. Use `team repair <team> --timeout-jobs --dry-run` when the same repair pass should also catch stale step-less jobs whose target instance belongs to the team.
