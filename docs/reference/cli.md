...
| `agent-team template smoke [ref] [--commands] [--format <template>] [--json]` | Render a template in a temp repo and run doctor, agent, pipeline, and team validation; add `--commands` for scoped follow-ups or `--strict-runtime` for CI runtime checks |
