...
| `agent-team snapshot diff <before.json> <after.json>` | Compare saved diagnostic artifacts, including provenance, git context, runtime profile, health, plan, next-action hints, instances, jobs, inboxes, queue, schedules, intake, events, and pipeline state |
