...
- **`agent-teamd`** — Go daemon, one per repo. Listens on `.agent_team/daemon.sock` and a bearer-token-protected `127.0.0.1:<port>` loopback HTTP endpoint advertised in `.agent_team/daemon/http.addr`. Static binary, no Python dependency.
...
All endpoints are the same JSON request/response surface on both transports: Unix socket at `.agent_team/daemon.sock` and loopback HTTP at the address in `.agent_team/daemon/http.addr`. The Unix socket stays the primary local/operator path. Loopback HTTP requires `Authorization: Bearer <token>` from either `.agent_team/daemon/operator.token` for CLI/operator calls or the per-instance token file in `.agent_team/state/<instance>/daemon.token`.

Loopback is same-host only and deliberately has no TLS. If the transport graduates beyond loopback, keep the same `/v1/...` resource surface and capability claims but replace same-host bearer-only transport security with mTLS, with SPIFFE/SPIRE as the likely identity substrate.
...
agent-team daemon adopt <instance> --pid <pid> [--agent manager] [--workspace <path>] [--runtime claude|codex|docker] [--session-id <id>] [--dry-run] [--format '{{.Metadata.Instance}} {{.Metadata.PID}}'] [--json]
...
agent-team runtime [--format '{{.Runtime}} {{.Available}}'] [--json] | set <claude|codex|docker> [--runtime-bin <path>] [--dry-run] [--json] | unset [--dry-run] [--json]
...
| `.agent_team/daemon/http.addr` | daemon (gitignored) | Actual loopback TCP address for the HTTP transport. Removed on graceful shutdown; recreated on next start. |
| `.agent_team/daemon/operator.token` | daemon (gitignored) | 0600 bearer token for CLI/operator loopback HTTP calls. |
...
For Claude, the daemon spawns `claude --session-id <uuid> <args...>`. The session UUID is generated by the daemon on `/v1/dispatch`, persisted, and reused on `/v1/start` via `claude --resume <uuid>` — so resume is deterministic without parsing claude's own output. Claude managed start regenerates the resume system prompt under `.agent_team/state/<instance>/runtime/system_prompt.md`; when a saved launch snapshot points at a transient prompt path, or when `/v1/start` receives `fresh: true`, it records `resume_fallback` and launches the declared instance fresh. For Codex, daemon-managed exec dispatches add the topology-resolved `--model <id>` selector and `-c model_reasoning_effort="<level>"`, add `--json`, tail `child.log` for the first `thread.started` JSONL event, and persist that `thread_id` as the session id. Codex managed start validates the workspace and the rollout under `~/.codex/sessions` (or `CODEX_HOME/sessions`) before running `codex exec --model <id> -c model_reasoning_effort="<level>" resume <session-id> -`; when preflight fails, it records `resume_fallback` and launches a fresh instance with the generated brief. Persisted runtime metadata records both effective model and effort, and `agent-team inspect` reports them.

`agent-team run --prompt/--detach/--attach` resolves the agent runtime client-side and sends full `--agents`, `--add-dir`, `--append-system-prompt-file`, env, stdin, and workspace values through `/v1/dispatch`. Event-driven ephemeral spawns now perform the same agent resolution inside the daemon: load `.agent_team/agents/<name>/agent.md`, build the `--agents` JSON, stage the unioned skills under the instance state dir, write the kickoff prompt file, create `.agent_team/state/<instance>/config.toml`, mint `.agent_team/state/<instance>/daemon.token`, and export `AGENT_TEAM_ROOT`, `AGENT_TEAM_INSTANCE`, `AGENT_TEAM_STATE_DIR`, `AGENT_TEAM_DAEMON_SOCKET`, `AGENT_TEAM_DAEMON_URL` when available, and `AGENT_TEAM_DAEMON_TOKEN_FILE` before dispatching. The token value itself is never exported. Codex dispatches use `codex exec --json -` plus stdin to avoid putting the full assembled agent prompt in argv and to expose the session id needed for managed resume.
