...
Agent resolution (loading `.agent_team/agents/<name>/agent.md`, building the `--agents` JSON, writing the kickoff prompt file, and `--add-dir`'ing the skills tmpdir) stays in `agent-team run` for SQU-28. Event-driven ephemeral spawns now create `.agent_team/state/<generated-instance>/`, write a resolved `config.toml` from repo config plus declared topology overrides, and export `AGENT_TEAM_ROOT`, `AGENT_TEAM_INSTANCE`, and `AGENT_TEAM_STATE_DIR` before dispatching. Full daemon-side launcher parity is still future work: either move the CLI launcher into a shared runtime package or teach event dispatch to send a fully resolved argv/env through `/v1/dispatch`.
