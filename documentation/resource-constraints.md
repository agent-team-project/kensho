...
- **Authority as topology.** Per-agent (or per-team) verb allowlists on the daemon API: workers get status/inbox/feedback/gate-report on their own job; reviewers add gate verdicts; managers hold the job lifecycle verbs; the auditor is read-only plus ticket filing. Identity is the instance metadata the daemon already holds per connection. This is blast-radius control for cooperating agents, not a security boundary against a hostile local process — state that honestly and design within it.
